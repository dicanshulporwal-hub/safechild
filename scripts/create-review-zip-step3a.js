...
const zipFile = path.join(releaseDir, 'safebrowse-stage11-step3a-review.zip');
...
const os = require('os');
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb_step3a_review_'));
...
try {
  ...
  const manifest = {
    stage: 'Stage 11',
    step: 'Step 3A - RBAC Critical Remediation',
    timestamp: new Date().toISOString(),
    branch,
    commitHash,
    archive: {
      filename: 'safebrowse-stage11-step3a-review.zip',
      sizeBytes: stat.size,
