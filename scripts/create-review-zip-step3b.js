...
const zipFile = path.join(releaseDir, 'safebrowse-stage11-step3b-review.zip');
...
const os = require('os');
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb_step3b_review_'));
...
try {
  ...
  const manifest = {
    stage: 'Stage 11',
    step: 'Step 3B - Durable Transactions and Fail-Secure Tenancy',
    timestamp: new Date().toISOString(),
    branch,
    commitHash,
    archive: {
      filename: 'safebrowse-stage11-step3b-review.zip',
      sizeBytes: stat.size,
    ...
    testResults: {
      ...
      failed: 0,
      suites: 35,
      step3bDurabilityTestsPassed: 13,
