...
const zipFile = path.join(releaseDir, 'safebrowse-stage11-step3c-review.zip');
...
const os = require('os');
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb_step3c_review_'));
...
try {
  ...
  const manifest = {
    stage: 'Stage 11',
    step: 'Step 3C - Single-Commit Transactions and Complete Tenancy Integrity',
    timestamp: new Date().toISOString(),
    branch,
    commitHash,
    archive: {
      filename: 'safebrowse-stage11-step3c-review.zip',
      sizeBytes: stat.size,
    ...
    probeResults: {
      ...
      ownershipTransferPerformedMultipleSaves: false,
      apiFailedButDiskCommittedOwnership: false,
      apiFailedButDiskContainsSuccessAudit: false,
      memoryDiskOwnerDiverged: false,
      rollbackPersistenceFailureSuppressed: false,
    },
    remediationProbeResults: {
    ...
    reviewStatus: 'VERIFIED_AWAITING_INDEPENDENT_AUDIT',
  };
